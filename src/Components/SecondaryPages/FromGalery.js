...
function FromGalery() {
  ...
  useEffect(()=>{
    ...
    axios.post('http://localhost/ultras/supPages/fetchAdminArtSupIMG.php', queryID)
    .then(response => {
      if(isMounted){
  ...
  const theOutputFromDB = datas.length ? datas.map((data,index) => {
    return    <div key={index} className="the_1main_cont">
                <p className="the_sup_title">{data.image_title}</p>
                <small>Published: {data.date}</small>
                <p className="the_sup_desc">{data.image_desc}</p>
                <div className="the_sup_images">
                  {data.image_src.split(',').map((image,index)=>{
                    return <div key={index} className="art_sup_media">
                      <img src={"http://localhost/ultras/uploaded_img/"+image} alt="Album" />
                    </div>
                  })}
                </div>
              </div>
  ...
  return (
    <div className="container supPageArt">
      ...
      <div className="newest_photos">
        {latestArticles}
      </div> {/* end Newest articles */}
    </div>
  );
}

export default FromGalery;